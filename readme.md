...
- **`data-accordion="$CONFIG"`** - for parent node.
- **`data-accordion-button`** - for button inside parent node.
- **`data-accordion-content`** - for content inside parent node.

> You can use multiple `data-accordion-button` and `data-accordion-content` inside an `data-accordion`.
> `$CONFIG` is *optional* the accordion config. Available config:
> 
> - **`effect`** - string effect name that handle expand/collapse event. Default effect is only adding class to each nodes. Ready-to-use effect is `simple`, thats only slideUp/slideDown the content.
